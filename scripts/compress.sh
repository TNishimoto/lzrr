...
for file in ${filearr[@]}; do
	echo "lz77 ${file}"
    nohup /usr/bin/time -f "#lz77 ${file}, %e sec, %M KB" ~/lzrr/build/compress.out -i ${folderpath}/${file} -o ${outputpath1}/${file}.lz77 -m lz >> /mnt/server/nishimoto/result/lzrr_output.log
done

for file in ${filearr[@]}; do
	echo "lex ${file}"
    nohup /usr/bin/time -f "#lex ${file}, %e sec, %M KB" ~/lzrr/build/compress.out -i ${folderpath}/${file} -o ${outputpath1}/${file}.lex -m lex >> /mnt/server/nishimoto/result/lzrr_output.log
done
